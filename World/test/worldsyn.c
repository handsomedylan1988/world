...
int main(int argc, char *argv[]) {
  ...
  FILE * fp_sp;
  FILE * fp_ap;
  fp_f0=fopen(argv[2],"rb");
  fp_sp=fopen(argv[3],"rb");
  fp_ap=fopen(argv[4],"rb");
  fs=atoi(argv[5]);
...
//
  ...
  fseek(fp_f0,0,SEEK_END);
  world_parameters.f0_length=ftell(fp_f0)/sizeof(double);
  fseek(fp_f0,0,SEEK_SET);
...
//
  ...
  for(int i=0;i<world_parameters.f0_length;i++)
  {
      world_parameters.spectrogram[i]=(double*)malloc(sizeof(double)*(world_parameters.fft_size/2+1));
      world_parameters.aperiodicity[i]=(double*)malloc(sizeof(double)*(world_parameters.fft_size/2+1));
  }

  fread(world_parameters.f0,sizeof(double),world_parameters.f0_length,fp_f0);
  for(int i=0;i<world_parameters.f0_length;i++)
  {
      fread(world_parameters.spectrogram[i],sizeof(double),world_parameters.fft_size/2+1,fp_sp);
      fread(world_parameters.aperiodicity[i],sizeof(double),world_parameters.fft_size/2+1,fp_ap);
  }
  fclose(fp_f0);
  fclose(fp_sp);
  fclose(fp_ap);
  ...
  // Output
  wavwrite(y, y_length, fs, 16, argv[1]);
