...
int main(int argc, char *argv[]) {
  ...
  FILE* fp_sp;
  FILE* fp_ap;
  fp_f0=fopen(argv[2],"wb");
  fp_sp=fopen(argv[3],"wb");
  fp_ap=fopen(argv[4],"wb");
  ...
  printf("fftsize:%d\n",world_parameters.fft_size);
  fwrite(world_parameters.f0,sizeof(double),world_parameters.f0_length,fp_f0);
  for(int i=0;i<world_parameters.f0_length;i++)
  {
      fwrite(world_parameters.spectrogram[i],sizeof(double),world_parameters.fft_size/2+1,fp_sp);
      fwrite(world_parameters.aperiodicity[i],sizeof(double),world_parameters.fft_size/2+1,fp_ap);
  }
  fclose(fp_f0);
  fclose(fp_sp);
  fclose(fp_ap);
